"""
Point-Based Shadow Ray Casting Engine - ENHANCED VERSION
=========================================================

Core shadow ray casting implementation with FIXED temporal metrics calculation
and TRUNK ZONE HANDLING for improved tree modeling.

ENHANCEMENTS:
- Trunk zone handling: rays below 25% of tree height can pass through if no other obstructions
- Differentiated tree/building obstruction logic
- Enhanced edge-to-points functionality
...
class ProductionShadowCaster:
    ...
    def cast_shadow_ray(self, point: Point, sun_pos: SunPosition, 
        ...
        while distance <= self.max_ray_distance:
            ...
            building_height = obstacle_heights.get('building', 0.0)
            canopy_height = obstacle_heights.get('canopy', 0.0)
            
            # Check for building obstruction (always blocks)
            if building_height > ray_z:
                return True  # Building blocks ray = point is in shadow
            
            # Check for canopy obstruction with trunk zone logic
            if canopy_height > ray_z:
                # Calculate trunk zone height (25% of canopy height)
                trunk_zone_height = canopy_height * 0.25
                
                # If ray is in trunk zone and no building obstruction, allow through
                if ray_z < trunk_zone_height:
                    # Ray is in trunk zone - continue checking for other obstructions
                    pass  
                else:
                    # Ray is in canopy zone - blocked
                    return True
