...
class SyntheticDSMDemo:
    ...
    def _create_synthetic_trees(self):
        ...
        for _ in range(20):
            ...
            # Create circular tree patch with correct indexing
                ...
                for dx in range(-radius, radius + 1):
                    if dx*dx + dy*dy <= radius*radius:
                        ...
                        if 0 <= y_idx < self.height and 0 <= x_idx < self.width:
                            distance_factor = np.exp(-(dx*dx + dy*dy) / (radius*radius / 2))
                            dsm[y_idx, x_idx] = max(dsm[y_idx, x_idx], tree_height * distance_factor)
...
def demo_performance_comparison():
    ...
    city = SyntheticDSMDemo(width=500, height=500)  # Smaller for faster demo
    
    # Generate test points (simulating edge/point locations)
    np.random.seed(42)
    n_points = 500  # Reduced for demo
    test_points_x = np.random.uniform(50, 450, n_points)
    test_points_y = np.random.uniform(50, 450, n_points)
    ...
    print(f"Full raster approach:  {estimated_raster_time:.0f} seconds (estimated)")
    print(f"Speedup factor:        {estimated_raster_time/point_time:.0f}x")
