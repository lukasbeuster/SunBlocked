...
from ray_caster_enhanced import ProductionShadowCaster, ShadeMetrics, batch_process_points_production
...
# Edge-to-Points Functionality
from shapely.geometry import LineString, Point
import geopandas as gpd

class EdgeToPointsProcessor:
    """Edge-to-points processing with shade analysis"""
    
    def __init__(self, dsm_tiles_directory: Union[str, Path],
                 lat: float = 42.36, lon: float = -71.06):
        """Initialize processor with enhanced ray caster"""
        self.shade_api = ThermalDriftShadeAPI(dsm_tiles_directory, lat, lon)
        self.lat = lat
        self.lon = lon
    
    def densify_edge_to_points(self, edge_geometry: LineString, 
                              spacing_m: float = 10.0,
                              edge_id: str = "unknown") -> List[Dict]:
        """Convert edge geometry to densified points
        
        Args:
            edge_geometry: LineString geometry to densify
        ...
        gdf = gpd.GeoDataFrame([{'geometry': edge_geometry}], crs='EPSG:4326')
        gdf_m = gdf.to_crs('EPSG:3857')  # Web Mercator for metric
        
        edge_m = gdf_m.geometry.iloc[0]
        length = edge_m.length
        
        if length == 0:
            return []
        
        # Generate points along the edge
        distances = np.arange(0, length + spacing_m, spacing_m)
        points_m = [edge_m.interpolate(float(d)) for d in distances]
        
        # Convert back to WGS84
        gdf_points_m = gpd.GeoDataFrame([{'geometry': p} for p in points_m], crs='EPSG:3857')
        gdf_points = gdf_points_m.to_crs('EPSG:4326')
        
        # Create point data
        point_data = []
        for i, point in enumerate(gdf_points.geometry):
            point_data.append({
                'point_id': f"{edge_id}_{i}",
                'edge_id': edge_id,
                'sample_index': i,
                'latitude': point.y,
                'longitude': point.x,
    ...
    def analyze_edge_shade_metrics(self, edge_geometry: LineString,
                                  current_time: datetime,
                                  spacing_m: float = 10.0,
                                  edge_id: str = "unknown") -> pd.DataFrame:
        """Analyze shade metrics for all points along an edge
        
        Args:
            edge_geometry: LineString geometry to analyze
            current_time: Current datetime for analysis
            spacing_m: Spacing between sample points in meters
            edge_id: Identifier for the edge
            
        Returns:
            DataFrame with shade metrics for each point along the edge
        """
        # Generate points along edge
        point_data = self.densify_edge_to_points(edge_geometry, spacing_m, edge_id)
        
        if not point_data:
            return pd.DataFrame()
        
        # Analyze shade metrics for each point
        results = []
        for point_info in point_data:
            try:
                # Use enhanced ray caster with trunk zone handling
                from ray_caster_enhanced import ProductionShadowCaster
                caster = ProductionShadowCaster(self.shade_api.dsm_manager)
                
                metrics = caster.compute_shade_metrics(
                    point=point_info['geometry'],
                ...
                # Convert to dictionary and add edge info
                result = {
                    'point_id': point_info['point_id'],
                    'edge_id': edge_id,
                ...
                print(f"⚠️ Error processing point {point_info['point_id']}: {e}")
                # Add default result for failed points
                results.append({
                    'point_id': point_info['point_id'],
                    'edge_id': edge_id,
                    'sample_index': point_info['sample_index'],
                    ...
                    'longitude': point_info['longitude'],
                    'current_shade': 0,
                    'last_1h_fraction': 0.0,
                    'last_2h_fraction': 0.0,
                    'last_4h_fraction': 0.0,
                    'since_dawn_fraction': 0.0,
                    'timestamp': current_time.isoformat(),
                    'error': str(e)
                })
    ...
    def process_multiple_edges(self, edges_gdf: gpd.GeoDataFrame,
                             current_time: datetime,
                             spacing_m: float = 10.0,
                             edge_id_column: str = 'edge_id') -> pd.DataFrame:
        """Process multiple edges from a GeoDataFrame
        
        Args:
            edges_gdf: GeoDataFrame with LineString geometries
            current_time: Current datetime for analysis
            spacing_m: Spacing between sample points in meters
            edge_id_column: Column name containing edge identifiers
            
        Returns:
            DataFrame with shade metrics for all points along all edges
        """
        all_results = []
        
        for idx, row in edges_gdf.iterrows():
            edge_id = row.get(edge_id_column, f"edge_{idx}")
            edge_geom = row.geometry
            
            if not isinstance(edge_geom, LineString):
                print(f"⚠️ Skipping non-LineString geometry for edge {edge_id}")
                continue
            
            print(f"🔄 Processing edge {edge_id}...")
            
            try:
                edge_results = self.analyze_edge_shade_metrics(
